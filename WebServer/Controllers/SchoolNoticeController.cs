...
namespace WebServer.Controllers
{
    ...
    [ApiController]
    public class SchoolNoticeController : ControllerBase
    {
        #region GET - API Key
        [HttpGet("{apiKey}")]
        public Dictionary<string, Dictionary<string, string>> Get(string apiKey)
        {
            ...
            try
            {
                ...
                Program.Logger.LogInformation("<" + clientInfo + "> 학교 공지사항 요청: 결과 - 000 (정상적으로 요청되었습니다.)");
                return Program.SchoolNotice;
            }
            catch (Exception e)
            {
                var errorDict = new Dictionary<string, Dictionary<string, string>>
