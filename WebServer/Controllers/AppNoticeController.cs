...
namespace WebServer.Controllers
{
    ...
    [ApiController]
    public class AppNoticeController : ControllerBase
    {
        #region GET - API Key
        [HttpGet("{apiKey}")]
        public Dictionary<string, Dictionary<string, string>> Get(string apiKey)
        {
            ...
            try
            {
                if (apiKey != Program.API_KEY)
                {
                    ...
                    Program.Logger.LogInformation("<" + clientInfo + "> 앱 공지사항 요청: 결과 - " + errorDict["Error"]["ResultCode"] + " (" + errorDict["Error"]["ResultMsg"] + ")");
                    return errorDict;
                }

                if(Program.AppNotice == null)
                {
                    var errorDict = new Dictionary<string, Dictionary<string, string>>
                    {
                        {
                ...
                Program.Logger.LogInformation("<" + clientInfo + "> 앱 공지사항 요청: 결과 - 000 (정상적으로 요청되었습니다.)");
                return Program.AppNotice;
            }
            catch (Exception e)
            {
                var errorDict = new Dictionary<string, Dictionary<string, string>>
                ...
                Program.Logger.LogError("<" + clientInfo + "> 앱 공지사항 요청: 결과 - 999 (" + e.Message + ")");
                return errorDict;
            }
        }
        #endregion
